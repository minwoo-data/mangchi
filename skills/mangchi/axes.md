...
지적해야 할 것:
- **중복 추가**: 같은 일을 하는 테이블/컬럼/함수/모듈/라이브러리가 **이미 존재**하는데 새로 만듦. (예: `pending_employee_names.resolved_email` 이미 있는데 `card_members.employee_email` 신설; 기존 util 함수 있는데 비슷한 helper 재작성)

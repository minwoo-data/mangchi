...
Claude에게 `auth.py`를 하드닝해달라고 함. Claude가 자기 코드를 자기가 리뷰하고 "이 정도면 됐다" 결론 → ship. 일주일 뒤, Codex나 다른 리뷰어가 Claude도 본인도 못 본 race condition을 즉시 발견. 같은 맹점, 한 모델, 한 pass.

**mangchi로:**

`/mangchi src/services/auth.py`. Round 1: Claude가 편집, Codex가 `correctness` 축 리뷰. Round 2: Claude 편집, Codex가 `security` 리뷰. Round 3: `readability`. 매 라운드마다 다른 축, 다른 모델이 비판. 2라운드 연속으로 "더 고칠 것 없음" 합의가 나거나 라운드 5 도달할 때까지 반복. delta 파일은 별도로 기록되어 원본은 명시적 지시 없으면 건드리지 않음.

## 누가 써야 하는가

- **출시 직전 프로덕션 코드** - 단일 파일을 5개 축에서 sweep
- **프롬프트 주입 / 보안 방어 코드** - 다른 모델의 adversarial 2차 검토
- **Claude Max 사용자 + Claude 토큰 여유** 가 있지만 외부 cross-model 리뷰가 필요하고 Codex를 수동으로 돌리긴 귀찮을 때
- **correctness / security / readability / performance / design** 한 번에 sweep
- **코드 리뷰 PR에 근거 문서 자동 첨부** - mangchi가 라운드별 history 작성

## 자매 도구 (같은 마켓플레이스)

- **[ddaro](https://github.com/minwoo-data/ddaro)** - worktree 기반 병렬 Claude Code 세션 + 안전한 merge.
- **[prism](https://github.com/minwoo-data/prism)** - 5-agent 병렬 코드 리뷰 (더 넓게, 1회성).
- **[triad](https://github.com/minwoo-data/triad)** - 디자인 문서 / markdown 3관점 숙의.
...
`mangchi` 는 haroom 플러그인 (ddaro, prism, triad) 과 함께 **haroom_plugins** aggregator 마켓플레이스를 통해 배포됩니다.
